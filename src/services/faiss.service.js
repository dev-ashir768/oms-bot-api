...
async function ingestDocuments(chunks) {
  const start = Date.now();
  if (!index) loadIndex();
...
async function searchSimilar(query, k = config.ragTopK) {
  const start = Date.now();
  ...
  const results = result.labels.map((idx, i) => ({
    text: documents[idx],
    score: result.distances[i]
  })).filter((r) => r.text !== void 0);
...
//# sourceMappingURL=faiss.service.js.map
